...
def main(args):
    ...
    # for x,y in model_without_ddp.named_parameters():
    #     print(x) # detr.backbone.0.body.layer3.3.conv3.weight 이런 형태 (body_d도 backbone에 속해있음)
        #print(y) # 실제 파라메터 텐서
    param_dicts = [
    ...
    #lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=25, T_mult=2, eta_min=0) #trash
    ...
    # print(len(next(iter(dataset_train))[0]))
    # print(next(iter(dataset_train))[0][0].shape, next(iter(dataset_train))[0][1].shape)
    ...
    if args.resume:
        ...
        if args.resume_reset:
            ...
            checkpoint['lr_scheduler']['last_epoch'] = 0
            checkpoint['lr_scheduler']['_step_count'] = 1
            

            print(checkpoint['lr_scheduler'])
            print(checkpoint.keys()) # dict_keys(['model', 'optimizer', 'lr_scheduler', 'epoch', 'args'])
            #del checkpoint['model']['detr.class_embed.weight']
            #del checkpoint['model']['detr.class_embed.bias']
            ##############################################################################    
